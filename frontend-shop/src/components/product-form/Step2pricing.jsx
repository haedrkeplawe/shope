...
/*
  Step2Pricing
  - الأسعار والتكلفة: سعر البيع (مطلوب)، سعر الشراء، السعر الأصلي
    ⚠️ سعر الشحن اتشال من هون نهائيًا - ما عاد بيتحدد لكل قطعة لحالها،
    بقى معتمد على منطقة الشحن (صفحة "الشحن والتوصيل" بالأدمن) ومحسوب
    على مستوى السلة كاملة وقت الدفع
  - إعدادات العرض والخصم: 4 مفاتيح تبديل + نسبة الخصم وتاريخ انتهائه
    (نسبة الخصم وتاريخه بيتعطلوا لو الخصم مش مفعّل)

  ⚠️ أضفنا "membersOnly" - مرتبط بفلتر "للأعضاء فقط" النظامي، بيحدد إذا كان
  هذا المنتج حصري للأعضاء بس عند عرضه بالمتجر
*/
      ...
      <div className="product-form-card">
        ...
        <div className="product-toggle-row">
          ...
          <ToggleSwitch
            ...
            onChange={(val) => updateField("isNewArrival", val)}
          />
        </div>

        <div className="product-toggle-row">
          <span className="product-toggle-label">
            حصري للأعضاء فقط (يظهر بفلتر "للأعضاء فقط")
          </span>
          <ToggleSwitch
            checked={formData.membersOnly}
            onChange={(val) => updateField("membersOnly", val)}
          />
        </div>
