...
/*
  ...
  ⚠️ إضافة: لو وصلنا هون قادمين من صفحة "المفضلة" (عبر SelectOfferModal
  بصفحة "العروض" → "إنشاء عرض جديد بدلاً من ذلك")، location.state.presetProduct
  بيحمل بيانات المنتج المختار سلفًا - بنجهّز الفورم تلقائيًا على
  targetType: "specific_products" مع هذا المنتج محدد مسبقًا، فالأدمن بس
  يكمل باقي تفاصيل العرض (الاسم، نسبة الخصم، التواريخ) بدل ما يعيد
  اختيار المنتج من الصفر. بيسري بس لحالة الإنشاء (مش التعديل)
*/
const OfferForm = () => {
  const { id } = useParams();
  ...
  const presetProduct = !isEdit ? location.state?.presetProduct : null;
  ...
  const [targetType, setTargetType] = useState(
    presetProduct ? "specific_products" : "all",
  );
