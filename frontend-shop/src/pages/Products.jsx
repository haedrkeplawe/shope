...
const Products = () => {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        const res = await fetch(`${API_URL}/products/filter-options`, {
          credentials: "include",
        });
        const result = await res.json();
        if (res.ok) setBrands(result.brands || []);
      } catch (error) {
        // تجاهل، الفلتر هيفضل يشتغل من غير خيارات ماركة جاهزة
      }
    };
    fetchBrands();
                  ...
                  <tr key={product._id}>
                    <td>
                      <div className="categories-table-name-image">
                        {primaryImage ? (
                          <img
                            src={`${ASSET_URL}${primaryImage.url}`}
                            alt={product.name}
                          />
                        ) : (
                          <FiImage />
