...
/*
  ...
  ⚠️ إضافة: لو وصلنا للصفحة قادمين من "المفضلة" (زر "إنشاء عرض" على منتج
  معيّن)، location.state.addProductToOffer بيحمل بيانات هيك منتج - بنفتح
  SelectOfferModal تلقائيًا يسمح للأدمن يختار عرض "منتجات محددة" جاهز
  يضيفه له، أو ينشئ عرض جديد من الصفر مع تجهيز المنتج مسبقًا بالفورم
*/
const Deals = () => {
  ...
  const [pendingProduct, setPendingProduct] = useState(
    location.state?.addProductToOffer || null,
  );
    ...
    <div className="deals-page">
      ...
      <SelectOfferModal
        ...
        offers={offers.filter(
          (o) => o.targetType === "specific_products" && o.status !== "expired",
        )}
        onClose={() => setPendingProduct(null)}
        onAdded={fetchOverview}
        onCreateNew={() =>
          navigate("/deals/new", { state: { presetProduct: pendingProduct } })
        }
      />
    </div>
  );
