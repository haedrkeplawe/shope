...
/*
  ...
  - بتجيب تفاصيل المنتج الكاملة + منتجات ذات صلة بطلب واحد متوازي، بنفس
    وقت فتح الصفحة
  - "أضف للسلة" مربوطة فعليًا بـ CartContext الحقيقي - بتبعت اللون
    والمقاس المختارين (لو المنتج بيدعمهم) والكمية
  ...
  - ⚠️ اللون قبل المقاس دائمًا (نفس ترتيب الاختيار المنطقي بمواقع الأزياء):
    الزبون بيحدد اللون الأول، وبعدين المقاس. كل سطر بالسلة/الطلب بيتحدد
    بالتركيبة الكاملة (منتج + مقاس + لون) - نفس المنتج بمقاس/لون مختلف
    بيصير سطر منفصل كليًا، مش بيندمج مع سطر موجود أصلاً (شوف CartContext
    وserializeCart.js بالباك اند لتفاصيل المنطق الكامل)
*/
const ProductDetails = () => {
  ...
  const fetchReviews = async () => {
    try {
      const res = await fetch(`${API_URL}/shop/products/${id}/rating/list`, {
        credentials: "include",
      });
      if (res.ok) {
      ...
      // تجاهل - قسم الآراء بيضل فاضي، مش خطأ حرج يمنع باقي الصفحة
    }
  };
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [detailRes, relatedRes, myRatingRes, reviewsRes] =
          await Promise.all([
            fetch(`${API_URL}/shop/products/${id}`, {
              credentials: "include",
            }),
            fetch(`${API_URL}/shop/products/${id}/related`, {
              credentials: "include",
            }),
            fetch(`${API_URL}/shop/products/${id}/rating/me`, {
              credentials: "include",
            }),
            fetch(`${API_URL}/shop/products/${id}/rating/list`, {
              credentials: "include",
            }),
          ]);
        ...
        const firstColor = detailData.product.colors?.[0];
        setSelectedColor(firstColor?.value || null);
        ...
        if (myRatingRes.ok) {
          ...
          setMyRating(myRatingData.myRating);
          setMyComment(myRatingData.myComment || "");
          setDraftRating(myRatingData.myRating || 0);
          setDraftComment(myRatingData.myComment || "");
        } else {
          setMyRating(null);
          setMyComment("");
          setDraftRating(0);
  ...
  const handleAddToCart = async () => {
    // لازم الزبون يختار لون ومقاس (لو المنتج بيدعمهم) قبل ما نضيف للسلة -
    // نفس التحقق موجود بالباك اند بشكل نهائي، بس هون منمنع الطلب أصلاً
    // ونوضح له بالضبط شو ناقص بدل ما ننتظر رد السيرفر
  ...
  // إرسال تقييم جديد أو تعديل تقييم موجود أصلاً (نفس الـ endpoint بالظبط -
  // الباك اند هو يلي بيقرر "إضافة" أو "تعديل" حسب لو الزبون قيّم قبل هيك).
  // التعليق النصي اختياري بالكامل - النجوم هي الإلزامية بس
  const handleSubmitRating = async () => {
    if (isSubmittingRating) return;
    if (!draftRating) {
      toast.error("الرجاء اختيار عدد النجوم أولًا");
      return;
    }

    setIsSubmittingRating(true);
    try {
      const res = await fetch(`${API_URL}/shop/products/${id}/rating`, {
        method: "POST",
        credentials: "include",
      ...
      if (!res.ok) {
        toast.error(data.message || "تعذّر حفظ التقييم");
        return;
      }
      ...
      // نعيد جلب قائمة "آراء الزبائن" من السيرفر عشان تنعكس فيها إضافة/تعديل
      // تقييمنا بشكل صحيح 100% (ترتيب، تاريخ، وباقي آراء الزبائن الآخرين)
      fetchReviews();
      ...
      <div className="pd-info">
        ...
        {product.ratingCount > 0 && (
          <div className="pd-rating-summary">
            <StarRating value={product.ratingAverage} size={15} />
            <span className="pd-rating-average">{product.ratingAverage}</span>
        ...
        {hasColors && (
          <div className="pd-color-section">
            <div className="pd-color-header">
              <span className="pd-color-label">اللون</span>
              {selectedColor && (
                <span className="pd-color-selected-name">
              ...
              {product.colors.map((c) => (
                <button
                  ...
                  className={`pd-color-swatch${
                    selectedColor === c.value ? " pd-color-swatch--active" : ""
                  }`}
                  style={c.colorHex ? { backgroundColor: c.colorHex } : {}}
                ...
                >
                  {!c.colorHex && (
                    <span className="pd-color-swatch-fallback">{c.label}</span>
                  )}
                </button>
              ))}
            </div>
        ...
        <div className="pd-rate-section">
          <h3 className="pd-section-title">
            <FiStar /> {myRating ? "تقييمك لهذه القطعة" : "قيّم هذه القطعة"}
          </h3>

          {/* ⚠️ تحديث الميزة الجديدة: إضافة تقييم تبقى تتطلب حساب مسجل
              دخوله (مرتبطة بحساب حقيقي بالباك اند - shop.routes.js) -
              زائر بلا حساب بيشوف دعوة لتسجيل الدخول بدل الودجت التفاعلي،
              بدل ما نخليه يعبّي نجوم وتعليق ثم يفشل الإرسال بلا داعي */}
          {isAuthenticated ? (
            <>
              <div className="pd-rate-widget">
                <StarRating
                  value={draftRating}
                ...
                />
                {draftRating > 0 && (
                  <span className="pd-rate-value">{draftRating} / 5</span>
                )}
              </div>

              <textarea
                className="pd-rate-comment-input"
                placeholder="شاركنا رأيك بهذه القطعة (اختياري)"
                value={draftComment}
                ...
                rows={3}
              />
              <span className="pd-rate-comment-counter">
                {draftComment.length}/500
              </span>

              <button
                type="button"
                className="pd-rate-submit-btn"
                onClick={handleSubmitRating}
                disabled={
                  isSubmittingRating ||
                  !draftRating ||
                  (draftRating === myRating &&
                    draftComment.trim() === myComment)
                }
              >
                {isSubmittingRating
                  ? "جاري الحفظ..."
            ...
            <p className="pd-rate-guest-note">
              <Link to="/login" className="pd-rate-guest-link">
                سجّل دخولك
              </Link>{" "}
              لتقييم هذه القطعة ومشاركة رأيك فيها
            </p>
          )}
        </div>
        ...
        {reviews.length > 0 && (
          <Accordion
            title={`آراء الزبائن (${reviews.length})`}
              ...
              {reviews.map((review) => (
                <div key={review.id} className="pd-review-item">
                  <div className="pd-review-header">
                    ...
                    <div className="pd-review-header-info">
                      <span className="pd-review-name">
                        {review.customerName}
                        {review.isMine && (
                          <span className="pd-review-mine-badge">تقييمك</span>
                        )}
                      </span>
