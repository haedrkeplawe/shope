...
/*
  ...
  ⚠️ تحديث الميزة الجديدة (تصفح بدون تسجيل دخول): لو الزائر وصل هون
  لأنه ضغط على صفحة محمية (مثلاً "إتمام الطلب")، بيرجعله على نفس
  الصفحة تلقائيًا بعد نجاح الدخول (redirectTo) بدل ما يضيع للرئيسية -
  وقبلها بيندمج سلته المحلية (لو عندها بنود) بحسابه الحقيقي تلقائيًا
  (mergeGuestCart من CartContext.jsx)
*/
const Login = () => {
  ...
  // ⚠️ مكان الصفحة الأصلية يلي الزائر انرجّع منها لتسجيل الدخول (مثلاً
  // /checkout) - محفوظة بـ state.from من ProtectedRoute.jsx، عشان
  // نرجّعه لنفس الصفحة بعد نجاح الدخول بدل ما يضيع دايمًا للرئيسية
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ومعه from عشان شاشة التحقق تقدر ترجّعه لنفس مكانه هو كمان
      navigate("/verify-otp", { state: { phone, otp: loginData.otp, from } });
    } catch (error) {
