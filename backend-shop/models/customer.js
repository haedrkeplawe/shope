...
const customerSchema = new mongoose.Schema(
  {
    ...
    // فوق (يلي مخصصة لتسجيل الدخول بس) - نفس فلسفة الفصل بين
    // passwordResetOtp وphoneOtp بالمشروع المرجعي (تطبيق المطعم)، عشان
    // طلب نسيان كلمة المرور ما يأثر أبدًا على أي جلسة OTP فعّالة لتسجيل
    // دخول جارٍ بنفس اللحظة، والعكس صحيح
    passwordResetOtp: {
      type: String,
      select: false,
    },
    ...
    /*
      ⚠️ الهدف من OTP بتسجيل الدخول هو التأكد من صلاحية الرقم مرة وحدة
      بس (أول عملية دخول ناجحة) - مش تحقق ثنائي (2FA) دائم بكل مرة.
      بمجرد ما ينجح أول verify-otp، بتصير true للأبد، وبعدها loginCustomer
      بيدخّل الزبون مباشرة بهاتف+كلمة مرور بدون أي رمز إضافي (شوف
      loginCustomer بالكنترولر). الأدمن/الموظف مختلفين عن قصد - عندهم
      OTP بكل مرة دايمًا (نفس نظام الأدمن الحالي، ما تغيّر شي هناك)
    */
    isPhoneVerified: {
      type: Boolean,
      default: false,
...
customerSchema.index(
  ...
  {
    unique: true,
    partialFilterExpression: { "marketer.code": { $type: "string" } },
  },
);
