...
const productSchema = new mongoose.Schema(
  {
    ...
    // ⚠️ لحظة أول نشر فعلي (publishStatus → "published") - أساس حساب
    // "الوصول المبكر" حسب باقة العضوية (شوف utils/membershipEngine.js).
    // بتتسجّل مرة وحدة بس أول ما ينشر المنتج فعليًا (createProduct/
    // updateProduct/updateProductStatus بـ product.controller.js) وما
    // بتتغيّر بعدها حتى لو اتخفي/انشر تاني - عشان ساعة الوصول المبكر ما
    // ترجع تصفر بدون قصد. بتخص بس منتجات membersOnly: true - المنتجات
    // العادية بتظهر فورًا لكل الزبائن بغض النظر عن هالحقل تمامًا متل
    // السلوك الحالي، بدون أي تغيير
    ...
    // متوسط التقييمات (0-5، بدقة عُشر واحد بعد التقريب) وعدد التقييمات -
    // Cache محسوب ومحدّث تلقائيًا من موديل Rating المنفصل (شوف
    // controllers/shop.controller.js → submitRating) في كل مرة يضيف/يعدّل
    // فيها زبون تقييمه، عشان نتجنب حساب المتوسط بـ aggregation كامل في كل
    // مرة حد يفتح صفحة المنتج (أداء أسرع بكتير لقراءة متكررة جدًا)
