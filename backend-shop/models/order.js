...
const orderSchema = new mongoose.Schema(
  {
    ...
    // -------------------- عضوية الولاء (Snapshot وقت الطلب) --------------------
    // مستقلة كليًا عن الكوبون فوق - بتتحسب وبتتطبق تلقائيًا حسب باقة
    // الزبون وقت الطلب بغض النظر عن وجود كوبون من عدمه (شوف شرح كامل
    // بـ utils/membershipEngine.js). membershipTierName نص ثابت (زي
    // marketerName بالأسفل) بيضل يعرض صح حتى لو الأدمن غيّر اسم الباقة
    // بعدين. membershipFreeShipping: true يعني الطلب استفاد من ميزة
    // التوصيل المجاني تبع باقته (shippingTotal صار صفر بسببها)
    membershipTierName: { type: String, default: null },
    membershipDiscount: { type: Number, default: 0 },
    membershipFreeShipping: { type: Boolean, default: false },

    // -------------------- المسوق بالعمولة (Snapshot وقت الطلب) --------------------
    // marketerId مرجع اختياري لحساب الزبون-المسوق يلي جاء الطلب عن طريق
    // رمزه (لو الأدمن ألغى تعيينه كمسوق بعدين يضل null بالمرجع بس
    // marketerName/marketerCode نص ثابت بيضل يعرض صح دايمًا) - نفس فلسفة
    // couponId/couponCode فوق بالضبط. commissionPercentage/commissionAmount
    // Snapshot ثابت لحظة الطلب - حتى لو الأدمن غيّر نسبة عمولة المسوق أو
    // حتى رمزه بعدين، الطلب القديم بيضل عارض بالضبط شو استحق المسوق وقتها
    //
    // ⚠️ لا يوجد علم "إلغاء/تأكيد" منفصل هون - إجمالي عمولة المسوق
    // (بلوحة الأدمن وبلوحة المسوق نفسه) بيُحسب لحظيًا من الطلبات بحالة
    // "delivered" فقط (نفس قاعدة totalSpent لعميل VIP بالضبط بـ
    // adminCustomer.controller.js) - أي طلب لسه قيد التنفيذ أو انلغى أو
    // ارتجع ما بيدخل بحساب "إجمالي العمولة" أبدًا، حماية لحق الأدمن لحد
    // ما يتأكد الطلب فعليًا وصل وانباع
    marketerId: {
      type: mongoose.Schema.Types.ObjectId,
      ref: "Customer",
      default: null,
    },
    marketerCode: { type: String, default: null },
    marketerName: { type: String, default: null },
    commissionPercentage: { type: Number, default: null },
    commissionAmount: { type: Number, default: 0 },

    grandTotal: { type: Number, required: true }, // = subtotal - couponDiscount - membershipDiscount + shippingTotal
