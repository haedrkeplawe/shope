...
/*
  بيبني خريطة value → {label, colorHex} لفلتر "اللون" تحديدًا - مختلفة عن
  buildLabelMap العادية لأنه اللون محتاج كمان الكود الست (colorHex) عشان
  نرسم دائرة اللون الحقيقية بواجهة السلة/الطلب، مش بس الاسم النصي

  بترجع كل قيم اللون المخزّنة (بدون فلترة active:true) عن قصد - لأنه ممكن
  الزبون يكون اختار لون قديم انحذف تفعيله لاحقًا من لوحة التحكم، وبالسلة/
  الطلب لازم يضل واضح شو اللون يلي اختاره فعليًا وقتها
*/
const buildColorMap = async () => {
  const filter = await AdvancedFilter.findOne({ key: "color" });
  if (!filter) return {};

  const map = {};
  filter.values.forEach((v) => {
    map[v.value] = { label: v.label, colorHex: v.colorHex || null };
  });
  return map;
};

module.exports = { buildLabelMap, buildLabelMaps, resolveLabel, buildColorMap };
