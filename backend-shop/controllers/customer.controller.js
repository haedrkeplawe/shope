...
// مدة صلاحية رمز "نسيت كلمة المرور" - مستقلة عن OTP_EXPIRE_MINUTES تبع
// تسجيل الدخول (مهلة أطول بشكل افتراضي لأنها خطوة أهم وأقل تكرارًا)
const PASSWORD_RESET_OTP_EXPIRE_MINUTES =
  Number(process.env.PASSWORD_RESET_OTP_EXPIRE_MINUTES) || 15;
...
// ⚠️ وضع تجريبي مؤقت (مرحلة الاختبار الحالية) - أي رمز OTP يتولّد
// (تسجيل دخول أو نسيان كلمة مرور) بيترجع صراحة بحقل "otp" برد الـAPI
// نفسه، بالإضافة للإرسال الفعلي عبر smsProvider - لأنه ممكن نستخدم أرقام
// وهمية بالاختبار أو تخلص رسائل الباقة. لإيقاف هالسلوك عند الانتقال
// للإنتاج الحقيقي: بس ضيف SMS_DEBUG_MODE=false بمتغيرات البيئة (بدون أي
// تعديل كود) - شوف نفس المتغير بـ store.controller.js
const SMS_DEBUG_MODE = process.env.SMS_DEBUG_MODE !== "false";
...
const serializeCustomer = (customer) => ({
  ...
  // ⚠️ عضوية الولاء - null يعني "العضوية المجانية" ضمنيًا (نفس فلسفة
  // membershipTierId بالموديل بالضبط). لازم customer يكون معمول له
  // .populate("membershipTierId") قبل ما توصل هون، وإلا بترجع null
  // دايمًا حتى لو الزبون فعليًا معيّن لباقة (شوف getMe تحت)
  membershipTierName:
    customer.membershipTierId && customer.membershipTierId.tierKey !== "free"
      ? customer.membershipTierId.displayName
      : null,
  membershipTierKey: customer.membershipTierId?.tierKey || "free",
});
...
exports.registerCustomer = async (req, res) => {
  try {
    ...
    // ⚠️ تحقق من صيغة الرقم قبل أي شي - Aman Gate بيرسل بس للأرقام
    // السورية (محليًا 09XXXXXXXX أو دوليًا 963+XXXXXXXXX)، فلو انسجّل
    // برقم بصيغة تانية، الزبون ما رح توصله رسائل SMS حقيقية أبدًا لاحقًا
    if (!smsProvider.isValidSyrianPhone(phone)) {
      return res.status(400).json({
        message: "رقم الهاتف غير صالح - الرجاء إدخال رقم سوري صحيح",
      });
    }
...
exports.loginCustomer = async (req, res) => {
  try {
    ...
    // ✅ الرقم موثّق من قبل (نجح OTP مرة وحدة سابقًا - isPhoneVerified) →
    // دخول مباشر بدون رمز جديد، بنفس شكل رد verifyOtp بالضبط (توكن +
    // كوكي) - الهدف من OTP كان التأكد من صلاحية الرقم مرة وحدة بس، مش
    // تحقق ثنائي دائم بكل تسجيل دخول
    if (customer.isPhoneVerified) {
      await customer.populate("membershipTierId", "displayName tierKey");
    ...
    // إرسال فعلي عبر Aman Gate - بمرحلة الاختبار الحالية (بناءً على طلب
    // صريح) ما منوقف الطلب إذا فشل الإرسال، لأنه ممكن نكون عم نجرب برقم
    // وهمي أو خلصت رسائل الباقة - بس رح نطبع تحذير بالكونسول
    const sent = await smsProvider.send(customer.phone, otpCode);
    if (!sent) {
      console.warn(
        `⚠️ فشل إرسال SMS لـ ${customer.phone} — مستمرّين (وضع اختبار)`,
      );
    }
    ...
    return res.status(200).json({
      ...
      // ⚠️ مؤقت لمرحلة الاختبار فقط - شوف شرح SMS_DEBUG_MODE بأعلى الملف
      ...(SMS_DEBUG_MODE ? { otp: otpCode } : {}),
    });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: "حدث خطأ في السيرفر" });
...
exports.verifyOtp = async (req, res) => {
  try {
    ...
    // ✅ أول تحقق ناجح من الرقم - من هلق وطالع، loginCustomer رح يدخّل
    // هالحساب مباشرة بدون طلب رمز جديد (شوف isPhoneVerified بالموديل)
...
/*
  -------------------- نسيت كلمة المرور --------------------
  خطوتين منفصلتين تمامًا عن تسجيل الدخول (حقول passwordResetOtp/
  passwordResetOtpExpire مستقلة عن otpCode/otpExpiresAt - شوف شرح موديل
  Customer): طلب رمز عبر الهاتف، ثم التحقق منه + تعيين كلمة مرور جديدة
  بخطوة واحدة (بدون رابط/توكن بالإيميل - الزبون أصلاً ما عنده إيميل حقيقي
  بالنظام، هاتف فقط)
*/

/*
  الخطوة الأولى: طلب رمز إعادة تعيين كلمة المرور
  POST /api/customers/forgot-password   body: { phone }
*/
exports.forgotPasswordCustomer = async (req, res) => {
  try {
    const { phone } = req.body;
    ...
    // ⚠️ ما منفرّق بالرسالة بين "رقم غير موجود" و"رقم موجود" لأي غرض أمني
    // إضافي هون مش ضروري حاليًا (نفس فلسفة باقي النظام - بساطة أولًا) -
    // بس منرجع نفس الرسالة الإيجابية لأي حالة، عشان ما يصير تعداد أرقام
    // (Phone Enumeration) من فرونت مفتوح للعامة
    if (!customer) {
      return res.status(200).json({
        message: "إذا كان الرقم مسجّلاً، رح توصلك رسالة فيها رمز التحقق",
      });
    }
    ...
    // نفس مبدأ الحماية من الإرسال المتكرر بفارق زمني قصير المستخدم بتسجيل
    // الدخول (otpLastSentAt) - هون على passwordResetOtpExpire نفسها بما
    // إنه ما في حقل passwordResetOtpLastSentAt منفصل: المهلة الكاملة
    // PASSWORD_RESET_OTP_EXPIRE_MINUTES ناقص OTP_RESEND_SECONDS هي أقل
    // وقت لازم يمر قبل ما نسمح بطلب رمز جديد
    if (customer.passwordResetOtpExpire) {
      const msRemaining =
        customer.passwordResetOtpExpire.getTime() - Date.now();
      const msSinceSent =
        PASSWORD_RESET_OTP_EXPIRE_MINUTES * 60 * 1000 - msRemaining;
      if (msSinceSent < OTP_RESEND_SECONDS * 1000 && msSinceSent >= 0) {
        const remaining = Math.ceil(
          (OTP_RESEND_SECONDS * 1000 - msSinceSent) / 1000,
        );
        return res.status(429).json({
          message: `الرجاء الانتظار ${remaining} ثانية قبل إعادة المحاولة`,
        });
      }
    }

    const otpCode = generateOtp();
    customer.passwordResetOtp = otpCode;
    customer.passwordResetOtpExpire = new Date(
      Date.now() + PASSWORD_RESET_OTP_EXPIRE_MINUTES * 60 * 1000,
    );
    await customer.save();
    ...
    if (!sent) {
      console.warn(
        `⚠️ فشل إرسال SMS (نسيان كلمة المرور) لـ ${customer.phone} — مستمرّين (وضع اختبار)`,
      );
    }
...
/*
  الخطوة الثانية: التحقق من الرمز + تعيين كلمة المرور الجديدة بخطوة واحدة
...
exports.resetPasswordCustomer = async (req, res) => {
  try {
    ...
    const isDevBypass = isDev && otp === DEV_OTP_BYPASS;

    if (!isDevBypass) {
      if (!customer.passwordResetOtp || !customer.passwordResetOtpExpire) {
        return res
          .status(400)
          .json({ message: "لا يوجد طلب فعّال، الرجاء طلب رمز جديد" });
      }
      if (customer.passwordResetOtpExpire < new Date()) {
        return res
          .status(400)
          .json({ message: "انتهت صلاحية الرمز، الرجاء طلب رمز جديد" });
      }
    ...
    customer.passwordResetOtpExpire = undefined;
    await customer.save();

    return res
      .status(200)
      .json({ message: "تم تعيين كلمة المرور الجديدة بنجاح" });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: "حدث خطأ في السيرفر" });
  }
...
/*
  -------------------- دمج سلة الزائر بحساب الزبون (ميزة جديدة) --------------------
  بتُستدعى مرة وحدة تلقائيًا من الفرونت (CartContext.jsx → mergeGuestCart)
  فور نجاح تسجيل الدخول أو التحقق من OTP، بالبنود يلي كانت مخزّنة محليًا
  كزائر قبل هيك (localStorage - شوف utils/guestCart.js بالفرونت، وشرح
  كامل لفلسفة سلة الزائر بالكامل بـ shop.controller.js → getCartPreview)

  ⚠️ منطق الدمج نفسه (تطابق منتج+مقاس+لون → زيادة الكمية، وإلا سطر جديد)
  بنفس فلسفة addToCart فوق بالضبط - بس هون بشكل مُجمّع (loop) بدل عنصر
  واحد بكل طلب. وبدون أي تحقق صارم من سقف المخزون هون بقصد - السقف
  الحقيقي أصلاً بيتطبق تلقائيًا لحظة أي قراءة تالية للسلة عبر
  buildCartResult (نفس فلسفة "بدون حالة مشتقة مخزّنة" المتبعة بكل
  النظام)، فمفيش داعي نكرر نفس التحقق هون بشكل مضاعف
  POST /api/customers/cart/merge   body: { items: [{ productId, size, color, quantity }] }
*/
exports.mergeCart = async (req, res) => {
  try {
    ...
    if (!Array.isArray(items) || items.length === 0) {
      return res.status(200).json({ message: "لا يوجد شيء لدمجه", count: 0 });
    }
    ...
    items.forEach((raw) => {
      if (!raw || !mongoose.Types.ObjectId.isValid(raw.productId)) return;

      const requestedQty = Math.max(1, Number(raw.quantity) || 1);
      const normalizedSize = raw.size || null;
      const normalizedColor = raw.color || null;
      const requestedVariant = { size: normalizedSize, color: normalizedColor };

      const existingLine = customer.cart.find(
        (item) =>
          item.productId.toString() === raw.productId &&
          sameVariant(item, requestedVariant),
      );

      if (existingLine) {
        existingLine.quantity += requestedQty;
      } else {
        customer.cart.push({
