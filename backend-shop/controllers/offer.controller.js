...
/*
  إضافة منتج واحد لعرض "منتجات محددة" موجود مسبقًا - مسار مختصر مستخدم
  من صفحة "المفضلة" بالأدمن (اختيار عرض جاهز لتطبيقه على منتج حفظه
  الزبائن بمفضلتهم، بدل إنشاء عرض جديد من الصفر في كل مرة). يشتغل بس مع
  عروض targetType === "specific_products" لأنها الوحيدة يلي تخزّن قائمة
  منتجات ممكن نضيف عليها عنصر مفرد بمنطقية
  PATCH /api/offers/:id/add-product   body: { productId }
*/
  ...
  try {
    ...
    if (offer.targetType !== "specific_products") {
      return res.status(400).json({
        message: "هذا العرض لا يستهدف منتجات محددة، لا يمكن إضافة منتج له",
      });
    }
    ...
    const alreadyIncluded = offer.productIds
      .map((p) => p.toString())
      .includes(productId.toString());

    if (!alreadyIncluded) {
      offer.productIds.push(productId);
      await offer.save();
    }
    ...
    return res.status(200).json({
      message: alreadyIncluded
        ? "المنتج مضاف على هذا العرض أصلًا"
        : "تمت إضافة المنتج على العرض بنجاح",
      offer: serializeOffer(populated),
    });
  } catch (error) {
    console.error(error);
