...
/*
  ⚠️ helper مستقل مستخدم من dashboard.controller.js كمان (كارت "السلات
  المتروكة" بلوحة التحكم الرئيسية) - نفس تعريف "سلة متروكة" بالضبط (آخر
  نشاط أقدم من ABANDONED_THRESHOLD_HOURS) عشان الرقم يطابق صفحة "السلات
  المتروكة" نفسها دائمًا (مصدر حقيقة واحد). نسخة أخف من
  getAbandonedCartsOverview تحت (بدون استبعاد السلات يلي كل بنودها غير
  متاحة فعليًا حاليًا عبر buildCartResult) - مناسبة لكارت إحصائية سريع،
  مش لعرض قائمة تفصيلية دقيقة 100%

  periodStart/prevPeriodStart (اختياريين): لو انمررو، بيرجع كمان كم سلة
  "دخلت" حالة الترك خلال الفترة الحالية مقابل السابقة (enteredCurrentPeriod/
  enteredPrevPeriod) - أساس حساب نسبة تغيّر حقيقية لكارت لوحة التحكم
*/
exports.getAbandonedCartsStats = async ({
  periodStart,
  prevPeriodStart,
} = {}) => {
  ...
  let count = 0;
  let enteredCurrentPeriod = 0;
  let enteredPrevPeriod = 0;
  ...
  customers.forEach((customer) => {
    ...
    if (periodStart && lastActivity >= periodStart.getTime()) {
      enteredCurrentPeriod += 1;
    } else if (
      periodStart &&
      prevPeriodStart &&
      lastActivity >= prevPeriodStart.getTime() &&
      lastActivity < periodStart.getTime()
    ) {
      enteredPrevPeriod += 1;
    }
  });

  return { count, enteredCurrentPeriod, enteredPrevPeriod };
};
