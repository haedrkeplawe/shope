...
// تعديل كود) - شوف نفس المتغير بـ customer.controller.js
...
exports.loginStore = async (req, res) => {
  try {
    ...
    const sent = await smsProvider.send(account.phone, otpCode);
    if (!sent) {
      console.warn(
        `⚠️ فشل إرسال SMS لـ ${account.phone} — مستمرّين (وضع اختبار)`,
      );
    }
...
/*
  -------------------- نسيت كلمة المرور --------------------
  موحّدة لحساب المالك وحسابات الموظفين سوا (findAccountByPhone فوق) -
  نفس فلسفة login/verify-otp/resend-otp بالضبط. خطوتين: طلب رمز عبر
  الهاتف، ثم التحقق منه + تعيين كلمة مرور جديدة بخطوة واحدة

  ⚠️ حقول passwordResetOtp/passwordResetOtpExpire مستقلة تمامًا عن
  otpCode/otpExpiresAt (تسجيل الدخول) - طلب نسيان كلمة المرور ما يأثر
  أبدًا على أي جلسة تسجيل دخول OTP فعّالة بنفس اللحظة، والعكس صحيح
*/

/*
  الخطوة الأولى: طلب رمز إعادة تعيين كلمة المرور
  POST /api/store/forgot-password   body: { phone }
*/
exports.forgotPassword = async (req, res) => {
  try {
    ...
    const found = await findAccountByPhone(phone, "+passwordResetOtpExpire");
    // ⚠️ نفس رسالة إيجابية موحّدة لأي حالة (رقم غير موجود/حساب موقوف)
    // عشان ما يصير تعداد أرقام (Phone Enumeration) من صفحة دخول عامة
    if (!found) {
      return res.status(200).json({
    ...
    // نفس مبدأ الحماية من الإرسال المتكرر المستخدم بتسجيل الدخول
    // (otpLastSentAt) - هون على passwordResetOtpExpire نفسها بما إنه ما
    // في حقل passwordResetOtpLastSentAt منفصل
    if (account.passwordResetOtpExpire) {
      const msRemaining = account.passwordResetOtpExpire.getTime() - Date.now();
      const msSinceSent =
    ...
    const otpCode = generateOtp();
    account.passwordResetOtp = otpCode;
    account.passwordResetOtpExpire = new Date(
    ...
    if (!sent) {
      console.warn(
        `⚠️ فشل إرسال SMS (نسيان كلمة المرور) لـ ${account.phone} — مستمرّين (وضع اختبار)`,
      );
    }
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    const found = await findAccountByPhone(
      phone,
    ...
    if (!isDevBypass) {
      if (!account.passwordResetOtp || !account.passwordResetOtpExpire) {
        return res
          .status(400)
          .json({ message: "لا يوجد طلب فعّال، الرجاء طلب رمز جديد" });
      }
      if (account.passwordResetOtpExpire < new Date()) {
        return res
          .status(400)
          .json({ message: "انتهت صلاحية الرمز، الرجاء طلب رمز جديد" });
      }
    ...
    account.passwordResetOtpExpire = undefined;
    await account.save();
