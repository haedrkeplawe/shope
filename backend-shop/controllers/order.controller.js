...
const { evaluateMembership } = require("../utils/membershipEngine");
const { evaluateReferralCode } = require("../utils/marketerEngine");
...
/*
  ...
  ⚠️ إضافة: التسويق بالعمولة - marketerCode (اختياري) بيجي منفصل تمامًا
  عن الكوبون، مش مخزّن على حساب الزبون إطلاقًا (بعكس appliedCoupon) -
  بيتحقق منه نهائيًا هون بس (utils/marketerEngine.js) لحظة تثبيت الطلب،
  وبيتجمّد كـ Snapshot على الطلب نفسه (marketerId/marketerCode/
  marketerName/commissionPercentage/commissionAmount) - شوف شرح موديل
  Order للتفاصيل الكاملة عن توقيت اعتماد العمولة نهائيًا
*/
exports.createOrder = async (req, res) => {
  try {
    const { shipping, shippingZoneId, marketerCode } = req.body;
    ...
    // -------------------- التحقق النهائي من رمز المسوّق (إن وجد) --------------------
    // نفس فلسفة الكوبون بالضبط - رمز المسوّق مش مخزّن على حساب الزبون
    // إطلاقًا (بعكس الكوبون)، بيوصل مباشرة مع طلب تثبيت الطلب نفسه من
    // خطوة المراجعة بالـ Checkout - قرار إسناد لمرة وحدة بس لحظة الشراء
    let marketerResult = null;
    if (marketerCode && marketerCode.trim()) {
      marketerResult = await evaluateReferralCode({
        code: marketerCode,
        subtotal,
        couponDiscount,
      });
      ...
      if (!marketerResult.valid) {
        return res.status(409).json({
          message: marketerResult.message || "رمز المسوّق غير صالح",
        });
      }
    }
    ...
    // ⚠️ لازم تُحسب بعد شحن عادي (فوق) عشان نعرف shippingResult.price
    // الحقيقي - محتاجينه لو باقة الزبون ما بتسمح بجمع الخصم مع التوصيل
    // المجاني سوا (allowStacking: false)، فمنحسم لصالح الأنفع للزبون
    // بالريال (شوف شرح كامل بـ utils/membershipEngine.js → evaluateMembership)
    const membershipResult = await evaluateMembership({
      customerId: req.customerAuth.id,
      subtotal,
      shippingPrice: shippingResult.price,
    });
    const membershipDiscount = membershipResult.discountAmount;
    const finalShippingTotal = membershipResult.freeShipping
      ? 0
      : shippingTotal;
          ...
          [
            {
              ...
              couponDiscount,
              // Snapshot عضوية الولاء وقت الطلب - شوف utils/membershipEngine.js
              membershipTierName:
                membershipResult.tier.tierKey !== "free"
                  ? membershipResult.tier.displayName
                  : null,
              membershipDiscount,
              membershipFreeShipping: membershipResult.freeShipping,
              // Snapshot المسوّق وقت الطلب - شوف utils/marketerEngine.js
              marketerId: marketerResult ? marketerResult.marketer._id : null,
              marketerCode: marketerResult
                ? marketerResult.marketer.marketer.code
                : null,
              marketerName: marketerResult
                ? marketerResult.marketer.fullName
                : null,
              commissionPercentage: marketerResult
                ? marketerResult.commissionPercentage
                : null,
              commissionAmount: marketerResult
                ? marketerResult.commissionAmount
                : 0,
              grandTotal:
                Math.max(0, subtotal - couponDiscount - membershipDiscount) +
                finalShippingTotal,
    ...
    // ⚠️ إشعار المسوّق بعملية بيع جديدة استخدمت رمزه - بعد نجاح الطلب
    // فعليًا. ملفوف بـ try/catch خاص فيه عشان فشل الإشعار (لو صار) ما
    // يوقف نجاح إنشاء الطلب نفسه أبدًا - نفس فلسفة adminOrder.controller.js
    if (marketerResult) {
      try {
        await createNotification({
          customerId: marketerResult.marketer._id,
          type: "marketer_sale",
          title: "🎉 عملية بيع جديدة عبر رمزك",
          message: `طلب جديد رقم ${order.orderNumber} استخدم رمز الإحالة الخاص بك - عمولة متوقعة ${marketerResult.commissionAmount.toLocaleString(
            "en-US",
          )} ل.س (تُعتمد نهائيًا بعد تسليم الطلب)`,
          link: "/account/marketer",
          relatedId: order._id,
        });
      } catch (notifyError) {
        console.error("تعذر إرسال إشعار المسوّق:", notifyError);
      }
    }
