...
const {
  ...
  getEffectiveTier,
} = require("../utils/membershipEngine");
...
exports.getNewArrivals = async (req, res) => {
  try {
    const rawProducts = await Product.find({ publishStatus: "published" })
      .sort({ createdAt: -1 })
      .limit(20); // نجيب أكتر من 4 قبل الفلترة عشان الحجب ما يخلينا نرجع أقل من المطلوب بدون داعي

    const products = (
      await filterVisibleForCustomer(rawProducts, req.customerAuth?.id)
    ).slice(0, 4);
...
/*
  قيم فلتر "نوع المنتج" (piece_type) النشطة بس - بعدّاد منتجات منشورة
  فعليًا لكل قيمة - نفس فلسفة getMainCategories فوق بالضبط، بس مبني على
  قيم فلتر AdvancedFilter مش على موديل Category (شوف seedDefaults.js
  للقيم الافتراضية: فستان/جاكيت/قميص...، والأدمن ممكن يضيف/يعدّل/يرتّب
  منها من صفحة "الفلاتر المتقدمة" بلوحة التحكم زي أي فلتر تاني)

  ⚠️ الصور: فلتر AdvancedFilter مالوش حقل صورة أصلاً (قيم نصية بس، شوف
  advancedFilter.js) - فبنرجّع value/label/productsCount بس، والفرونت
  (ShopByPieceType.jsx) هو يلي فيه خريطة صور محلية (قيمة → رابط صورة)
  بيعبّيها صاحب المشروع يدويًا

  قسم "تسوق حسب الفئة" الجديد بالصفحة الرئيسية، تحت قسم "تصفح حسب
  الفئة" (ShopByCategory) مباشرة - نفس آلية العمل بالضبط: الضغط على أي
  كارت بيودّي لصفحة المتجر مع تفعيل فلتر piece_type تلقائيًا
  GET /api/shop/piece-types
*/
exports.getPieceTypes = async (req, res) => {
  try {
    const filterDoc = await AdvancedFilter.findOne({ key: "piece_type" });
    if (!filterDoc || !filterDoc.active || filterDoc.values.length === 0) {
      return res.status(200).json({ pieceTypes: [] });
    }

    const activeValues = filterDoc.values
      .filter((v) => v.active)
      .sort((a, b) => a.order - b.order);

    const field = filterDoc.productField || "pieceType";
    ...
    const pieceTypes = activeValues.map((v) => ({
      value: v.value,
      label: v.label,
      productsCount: countMap[v.value] || 0,
    }));
...
/*
  أفضل 4 آراء عملاء لقسم "آراء عملائنا" بالصفحة الرئيسية - شرطين إلزاميين
  بالضبط زي ما طلب صاحب المشروع:
  1) موافق عليها من الأدمن حصرًا (approvalStatus: "approved") - أي تقييم
     "قيد المراجعة" أو "مرفوض" ما بيظهر هون أبدًا (نفس فلسفة كل مكان تاني
     بالسستم بيعرض تقييمات للعامة)
  2) الأعلى تقييمًا (ترتيب تنازلي بالنجوم، والأحدث أولاً كفاصل عند تساوي
     النجوم) - ومعها تعليق نصي فعلي (مفيش داعي نعرض تقييم بنجوم بس بدون
     كلام بقسم "تجارب حقيقية" مخصص عرض آراء مكتوبة)

  ⚠️ "المدينة" الظاهرة تحت اسم الزبون: موديل Customer نفسه مالوش حقل
  مدينة أصلاً (شوف customer.js) - المدينة الوحيدة المتوفرة هي Snapshot
  عنوان الشحن المحفوظ بآخر طلب حقيقي للزبون (order.shipping.city). لو
  الزبون قيّم منتج بدون ما يكون طلب أي شي أبدًا (ممكن تقنيًا، التقييم مش
  مربوط بشراء فعلي)، بترجع مدينة فاضية والفرونت بيتجاهل عرضها بس، بدون
  ما يكسر الكارت
  GET /api/shop/testimonials
*/
exports.getTestimonials = async (req, res) => {
  try {
    ...
    const validRatings = topRatings.filter((r) => r.customerId);
    ...
    const testimonials = await Promise.all(
      validRatings.map(async (rating) => {
        ...
        return {
          id: rating._id,
          customerName: rating.customerId.fullName,
...
exports.getProductById = async (req, res) => {
  try {
    ...
    // ⚠️ منتج "للأعضاء فقط" لسه بفترة الوصول المبكر عن باقة الزبون -
    // بنرجّع 404 (مش 403) بقصد، نفس فلسفة أي حالة داخلية تانية فوق
    // بالضبط - ما بدنا نسرّب حتى وجود المنتج لزبون مش مؤهل يشوفه لسه
    const tier = await getEffectiveTier(req.customerAuth?.id);
    if (!isProductVisibleToCustomer(product, tier)) {
      return res.status(404).json({ message: "المنتج غير موجود" });
    }
    ...
    // ⚠️ عداد المشاهدات (تكرار محجوب لكل زبون) بيتحسب بس لزبون معروف -
    // زائر بلا حساب مالوش هوية ثابتة نربط فيها "شافها قبل" من عدمه، فبنكتفي
    // بعدم احتساب مشاهدات الزوار بهاد العداد بدل ما نعتمد بديل مؤقت
    // (IP، جلسة...) خارج نطاق هاي الميزة حاليًا
...
exports.getRelatedProducts = async (req, res) => {
  try {
    ...
    // ⚠️ فلترة أخيرة حسب الوصول المبكر لمنتجات "للأعضاء فقط" - ممكن
    // يقل العدد عن 4 بحالات نادرة (لو من ضمن "ذات صلة" منتج عضوية لسه
    // بفترة الحجب)، مقبول لواجهة اختيارية زي هاي
    const visibleRelated = await filterVisibleForCustomer(
      related,
      req.customerAuth?.id,
    );

    const withPricing = await attachEffectivePrice(visibleRelated);
    const labelMaps = await buildLabelMaps(["condition", "brand"]);
    const serialized = withPricing.map((p) =>
      serializeShopProduct(p, labelMaps),
    );
...
exports.getMyRating = async (req, res) => {
  try {
    ...
    // زائر بلا حساب مالوش "تقييمي الشخصي" أصلاً - بنرجّع فاضي فورًا
    // بدون أي استعلام، بدل ما نرفض الطلب (هاد الراوت بقى مفتوح للزائر
    // كمان - شوف shop.routes.js)
    if (!req.customerAuth?.id) {
      return res.status(200).json({ myRating: null, myComment: "" });
    }
...
exports.getProductRatings = async (req, res) => {
  try {
    ...
    // الزبون الحالي نفسه دايمًا لو مسجل دخوله (حتى لو لسه قيد المراجعة
    // أو مرفوض) عشان يشوف تقييمه هو نفسه بأي حالة - زائر بلا حساب بيشوف
    // المعتمد بس، منطقيًا (ما إله "تقييمي أنا" أصلاً)
    const myId = req.customerAuth?.id ? req.customerAuth.id.toString() : null;

    const ratings = await Rating.find({
      productId: id,
      $or: myId
        ? [{ approvalStatus: "approved" }, { customerId: myId }]
        : [{ approvalStatus: "approved" }],
    })
      .sort({ createdAt: -1 })
      .limit(LIMIT)
    ...
    const reviews = ratings.map((r) => ({
      ...
      isMine: myId ? r.customerId?._id?.toString() === myId : false,
    }));
...
exports.getShopProducts = async (req, res) => {
  try {
    ...
    // ⚠️ حجب منتجات "للأعضاء فقط" (membersOnly: true) لسه بفترة الوصول
    // المبكر عن باقة الزبون - قبل أي شي تاني (سعر/ترتيب/تصفيح) عشان
    // إجمالي النتائج (total/totalPages) يعكس فقط المنتجات المسموح له
    // يشوفها فعليًا - شوف utils/membershipEngine.js
    const visibleProducts = await filterVisibleForCustomer(
      rawProducts,
      req.customerAuth?.id,
    );
    let candidates = await attachEffectivePrice(visibleProducts);
...
/*
  -------------------- عضوية الولاء (واجهة المتجر) --------------------
*/

/*
  منتجات "membersOnly" لسه بفترة الوصول المبكر - مؤهل الزبون يشوفها
  حسب باقته بس لسه ما ظهرت للباقة المجانية (أساس قسم "وصل حديثًا
  للأعضاء المميزين"). زبون بالباقة المجانية بيرجع له دايمًا مصفوفة فاضية
  (منطقيًا - القسم كله مخصص لصاحب باقة أعلى منها بس)
  GET /api/shop/early-access
*/
exports.getEarlyAccessProducts = async (req, res) => {
  try {
    ...
    const visible = await filterEarlyAccessOnly(
      rawProducts,
      req.customerAuth?.id,
    );

    const withPricing = await attachEffectivePrice(visible);
    const labelMaps = await buildLabelMaps(["condition", "brand"]);
    const serialized = withPricing.map((p) =>
      serializeShopProduct(p, labelMaps),
    );
...
/*
  عرض الباقات الثلاث للزبون (صفحة معلوماتية بس - بلا أي تفعيل/إلغاء
  ذاتي، شوف شرح كامل بـ adminMembership.controller.js) + باقته الحالية
  إن وجدت. الباقة المجانية معروضة برسالتها الخاصة ("الحالة الطبيعية")
  مش كخيار "اشترك فيها"
  GET /api/shop/membership-tiers
*/
exports.getMembershipTiersForCustomer = async (req, res) => {
  try {
    const tiers = await MembershipTier.find({}).sort({
      earlyAccessDelayHours: 1, // gold (0) ثم silver ثم free
    });

    const currentTier = await getEffectiveTier(req.customerAuth?.id);
    ...
    return res.status(200).json({
      tiers: tiers.map((t) => ({
        ...
        minPurchaseForDiscount: t.minPurchaseForDiscount,
        freeShippingMinOrder: t.freeShippingMinOrder,
        allowStacking: t.allowStacking,
        earlyAccessDelayHours: t.earlyAccessDelayHours,
        isActive: t.isActive,
      })),
...
/*
  ...
  ==================================================================
  ⚠️ الميزة الجديدة (تصفح + إضافة للسلة بدون تسجيل دخول): السلة الحقيقية
  مخزّنة أصلاً بحساب الزبون (customer.cart بموديل Customer)، وزائر بلا
  حساب مالوش أي حساب يتخزن فيه شي - فسلة الزائر بالكامل عايشة بالفرونت
  (localStorage بس، شوف utils/guestCart.js بالفرونت). هاد الـ endpoint
  بياخد نفس شكل بنود السلة الخام (productId/size/color/quantity + معرّف
  سطر جاي من الفرونت بدل ما يكون _id مستند حقيقي) وبيحسبلها بالضبط نفس
  التسعير اللحظي والتحقق من التوفر يلي بتاخده سلة الزبون المسجل -
  utils/serializeCart.js نفسها بالحرف، صفر تكرار منطق تسعير

  ⚠️ عن قصد بدون دعم كوبون هون: الكوبونات مرتبطة بحساب زبون حقيقي دايمًا
  (حد استخدام لكل زبون، نطاق specific_customers...) فما إلها معنى لزائر
  بلا هوية. الزائر بيشوف سعره بعد العروض العامة بس (applyOffers) بدون
  خصم عضوية (باقته الفعلية "مجانية" تلقائيًا زي أي زائر - membershipEngine.js
  نفسها بتتعامل معه هيك أصلاً). لحظة ما يسجّل دخول، سلته بتنتقل لحسابه
  تلقائيًا (customer.controller.js → mergeCart) وتصير خاضعة لكل الميزات
  الكاملة (كوبون، خصم عضوية...) عادي
  POST /api/shop/cart/preview   body: { items: [{ lineId, productId, size, color, quantity }] }
*/
exports.getCartPreview = async (req, res) => {
  try {
    ...
    if (!Array.isArray(items)) {
      return res.status(400).json({ message: "شكل بيانات السلة غير صالح" });
    }

    // بنبني نفس شكل rawCart يلي buildCartResult متوقعته بالضبط (نفس شكل
    // customer.cart الحقيقي) - lineId جاي من الفرونت (مولّد محليًا
    // بـ localStorage عند الإضافة) هو بديل الـ _id الحقيقي هون بس
    const rawCart = items
      .filter((item) => item && mongoose.Types.ObjectId.isValid(item.productId))
      .map((item) => ({
        _id: item.lineId || item.productId,
    ...
    const result = await buildCartResult(rawCart, {
      customerId: req.customerAuth?.id || null,
      appliedCouponCode: null, // بلا كوبون عن قصد - شوف الشرح فوق
    });
