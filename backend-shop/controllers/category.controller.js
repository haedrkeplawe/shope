...
exports.createCategory = async (req, res) => {
  try {
    ...
    // req.file.path = رابط الصورة الكامل على Cloudinary
    // req.file.filename = الـ public_id (لازم نخزنه عشان نقدر نحذف الصورة لاحقًا)
...
exports.updateCategory = async (req, res) => {
  try {
    ...
    if (req.file) {
      // احذف الصورة القديمة من Cloudinary قبل ما نستبدلها، عشان منسيبش ملفات يتيمة
      if (category.imagePublicId) {
        await cloudinary.uploader
          .destroy(category.imagePublicId)
          .catch((err) =>
...
exports.deleteCategory = async (req, res) => {
  try {
    ...
    // امنع الحذف لو الفئة مرتبطة بعرض نشط أو مجدول (لسه ما خلص تاريخه ومش متوقف)
    // عشان ما يضل عندنا عرض معلّق بفئة محذوفة (Dangling Reference)
    ...
    if (linkedOffer) {
      return res.status(400).json({
        message: `لا يمكن حذف هذه الفئة لأنها مرتبطة بعرض نشط أو مجدول ("${linkedOffer.title}") - أوقف العرض أو غيّر فئته أولًا`,
      });
    }
    ...
    // احذف صورة الفئة من Cloudinary لو موجودة، قبل حذف الفئة نفسها
    if (category.imagePublicId) {
      await cloudinary.uploader
        .destroy(category.imagePublicId)
        .catch((err) => console.error("فشل حذف صورة الفئة:", err.message));
    }
