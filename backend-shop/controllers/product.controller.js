...
/*
  جلب إحصائيات المخزون (لصفحة "المخزون وحالة القطع")
  GET /api/products/inventory-overview
  - بيرجع عدّاد كل حالة من حالات القطعة + الإجمالي الكلي (شامل مسودة/مخفي)
*/
exports.getInventoryOverview = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      stats: {
        awaitingPhotos: countMap.awaiting_photos || 0,
        underReview: countMap.under_review || 0,
        available: countMap.published || 0,
        reserved: countMap.reserved || 0,
        sold: countMap.sold || 0,
        draft: countMap.draft || 0,
        hidden: countMap.hidden || 0,
        total,
      },
    });
  } catch (error) {
