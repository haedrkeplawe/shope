...
/* -------------------- التسويق بالعمولة (لوحة المسوّق) --------------------
   محمية بنفس verifyCustomer العادية - التحقق من isMarketer نفسه بيصير
   جوا الكنترولر (403 لو الحساب مش مسوّق أصلاً) */
router.get("/marketer/me", verifyCustomer, getMyMarketerProfile);
...
// ⚠️ دمج سلة الزائر (localStorage) بحساب الزبون فور تسجيل الدخول - جزء
// من ميزة "تصفح بدون تسجيل دخول" الجديدة (شوف شرح كامل بالكنترولر).
// لازم تتسجّل هون *قبل* /cart/:productId بالضبط لنفس سبب ترتيب راوت
// الكوبون فوق (Express بيطابق "merge" كأنه productId فعلي لو كانت
// /cart/:productId مسجّلة قبلها)
router.post("/cart/merge", verifyCustomer, mergeCart);
